 ...
 * @since 6.1.0
 */
fun CharSequence.parseToLocalDateTime(): LocalDateTime {
 ...
 * @since 6.1.0
 */
fun CharSequence.parseToLocalDateTimeOrError() = either {
    catching({ this@parseToLocalDateTimeOrError.parseToLocalDateTime() }) { e: DateTimeParseException ->
        InvalidFormat(e.parsedString, LocalDateTime::class)
    }
}
 ...
 * @since 6.1.0
 */
fun CharSequence.parseToOffsetDateTime(): OffsetDateTime {
    if (isEmpty()) throw DateTimeParseException("The input is empty.", toString(), 0)
    val dateTimeString = toString().trim()

    return dateTimeString.parseToLocalDateTime().atOffset(
...
/**
 * Parses the [CharSequence] into an [OffsetDateTime], or returns a [InvalidFormat] wrapped in an [Either].
 *
 * This function attempts to parse the [CharSequence] using the [parseToOffsetDateTime] function.
 * If parsing fails with a [DateTimeParseException], it catches the exception and converts it into
 * a [InvalidFormat] with details about the failure (e.g., the invalid string and error index).
 ...
 *         - A [Either.Left] with a [InvalidFormat] if parsing fails.
 * @since 6.1.0
 */
fun CharSequence.parseToOffsetDateTimeOrError() = either {
    catching({ this@parseToOffsetDateTimeOrError.parseToOffsetDateTime() }) { e: DateTimeParseException ->
        InvalidFormat(e.parsedString, OffsetDateTime::class)
    }
}
 ...
 * @since 6.1.0
 */
fun CharSequence.parseToLocalDate(): LocalDate {
    if (isEmpty()) throw DateTimeParseException("The input is empty.", toString(), 0)
    parseToLocalDateTimeOrError().onRight { return it.toLocalDate() }
 ...
 * and encapsulates the parsing error in a [InvalidFormat].
 *
 ...
 * [InvalidFormat] describing the parsing failure on the left.
 * @throws IllegalArgumentException If the [CharSequence] is invalid for parsing and
 * the exception is not handled within the [catching] block.
 *
 * @since 6.1.0
 */
fun CharSequence.parseToLocalDateOrError() = either {
    catching({ this@parseToLocalDateOrError.parseToLocalDate() }) { e: DateTimeParseException ->
        InvalidFormat(e.parsedString, LocalDate::class)
    }
}
 ...
 * @since 6.1.0
 */
fun CharSequence.parseToLocalTime(): LocalTime {
    if (isEmpty()) throw DateTimeParseException("The input is empty.", toString(), 0)
...
/**
 * Parses the current [CharSequence] into a [LocalTime] object or returns a [InvalidFormat] in case of failure.
 *
 * This method attempts to parse the [CharSequence] using ISO 8601 time and date-time formats by invoking
 * the [parseToLocalTime] method. If the parsing operation fails and throws a [DateTimeParseException],
 * it maps the exception into a [InvalidFormat] containing details about the invalid input,
 ...
 * @throws RaiseSignal If there is a signal for unwinding the stack during error handling.
 * @since 6.1.0
 */
fun CharSequence.parseToLocalTimeOrError() = either {
    catching({ this@parseToLocalTimeOrError.parseToLocalTime() }) { e: DateTimeParseException ->
        InvalidFormat(e.parsedString, LocalTime::class)
    }
}
 ...
 * @since 6.1.0
 */
fun CharSequence.parseToOffsetTime(): OffsetTime {
    if (isEmpty()) throw DateTimeParseException("The input is empty.", toString(), 0)
    val dateTimeString = toString().trim()

    return dateTimeString.parseToLocalTime().atOffset(
        if ("Z" in dateTimeString) ZoneOffset.UTC
        else ZoneOffset.of(
            dateTimeString.substring(
 ...
 * @since 6.1.0
 */
fun CharSequence.parseToOffsetTimeOrError() = either {
    catching({ this@parseToOffsetTimeOrError.parseToOffsetTime() }) { e: DateTimeParseException ->
        InvalidFormat(e.parsedString, OffsetTime::class)
    }
}
...
/**
 * Executes the specified action if the current object is before the provided `other` object
 * based on the implementation of the `Comparable` and `Temporal` interfaces.
 *
 * @param other the object to compare the current object with.
 * @param action a function that consumes a pair containing the current object and `other` if the condition is met.
 * @return the current object.
 * @since 5.0.0
 */
inline fun <T> T.ifBefore(other: T, action: Consumer<MonoPair<T>>): T where T : Comparable<T>, T : Temporal {
    if (checkIsBefore(other)) action(MonoPair(this, other))
    return this
}
/**
 * Executes the specified action if the current temporal value is not before the specified `other` value.
 *
 * @param other the value to compare against the current instance.
 * @param action the action to be executed if the current instance is not before the `other` value.
 * @return the current instance.
 * @since 5.0.0
 */
inline fun <T> T.ifNotBefore(other: T, action: Consumer<MonoPair<T>>): T where T : Comparable<T>, T : Temporal {
    if (!checkIsBefore(other)) action(MonoPair(this, other))
    return this
}
/**
 * Executes a specified action if the current object is after the given `other` object
 * according to their natural ordering and temporal comparison.
 *
 * @param other the object to compare against the current object.
 * @param action the action to perform if the current object is after `other`.
 *               The action receives a `MonoPair` containing the current object as the first
 *               element and `other` as the second element.
 * @return the current object.
 * @since 5.0.0
 */
inline fun <T> T.ifAfter(other: T, action: Consumer<MonoPair<T>>): T where T : Comparable<T>, T : Temporal {
    if (checkIsAfter(other)) action(MonoPair(this, other))
    return this
}
/**
 * Executes the given action if the current object is not after the specified `other` object.
 *
 * @param other the object to compare with the current object. Must be of a type that is both
 *              comparable and temporal.
 * @param action a consumer that accepts a `MonoPair` of the current object and the `other` object
 *               when the current object is not after the `other` object.
 * @return the current object.
 * @since 5.0.0
 */
inline fun <T> T.ifNotAfter(other: T, action: Consumer<MonoPair<T>>): T where T : Comparable<T>, T : Temporal {
    if (!checkIsAfter(other)) action(MonoPair(this, other))
    return this
}
/**
 * Executes a specified action if this temporal object is equal to the given temporal object.
 * The equality check is based on the type and value of the temporal objects.
 *
 * @param other the temporal object to compare with this object.
 * @param action a consumer receiving a pair of the compared objects if they are considered equal.
 * @return the original temporal object (`this`) on which the method was invoked.
 * @since 5.0.0
 */
inline fun <T> T.ifTemporalEquals(other: T, action: Consumer<MonoPair<T>>): T where T : Comparable<T>, T : Temporal {
    if (checkEquality(other)) action(MonoPair(this, other))
    return this
}
/**
 * Executes the given action if the current temporal object is not equal to the specified temporal object.
 * Temporal equality is determined using the `checkEquality` function.
 *
 * @param other the temporal object to compare with the current object.
 * @param action the action to perform if the objects are not equal. It receives a `MonoPair` containing the current object and the other object.
 * @return the current temporal object.
 * @since 5.0.0
 */
inline fun <T> T.ifTemporalNotEquals(other: T, action: Consumer<MonoPair<T>>): T where T : Comparable<T>, T : Temporal {
    if (!checkEquality(other)) action(MonoPair(this, other))
    return this
}