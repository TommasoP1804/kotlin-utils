...
class Cuid private constructor(private val value: String, val version: CuidVersion = identifyVersion(value), private val savedTimestamp: Long? = null) : CharSequence, Serializable {
    /**
     * Represents a computed timestamp value associated with the CUID instance.
     *
     * The `timestamp` property provides a means of extracting the timestamp from the
     * internally stored CUID `value`. For CUIDs generated using version `CUIDv1`, the
     * timestamp is derived from a specified portion of the CUID's `value` and is returned
     * as a `Long` value in the base defined by the class.
     *
     * For CUIDs generated using version `CUIDv2`, the availability of the timestamp depends on
     * whether it was generated internally by this class. If the timestamp cannot be computed
     * or was not saved during the generation of the instance, an exception of type
     * `Uncomputable` is raised.
     *
     * This implementation ensures compatibility between different CUID versions while
     * retaining the ability to provide timestamp metadata when available.
     *
     * @return An `Either` instance that contains the timestamp as a `Long` or an
     * `Uncomputable` error when unavailable.
     * @since 6.1.0
     */
    val timestamp: Either<Uncomputable, Long>
        get() = either {
            if (version == CuidVersion.V2)
                savedTimestamp ?: raise(Uncomputable("The timestamp property is not available for CUIDv2 not generated with this class"))
            else value[1..<9].toLong(BASE)
        }

    /**
     * Represents either an `Uncomputable` result or a computed `Instant` object derived
     * from the `timestamp` property.
     *
     * This property provides a mapping of the internal `timestamp` value to an `Instant`
     * using a transformation function. If the mapping operation fails, the result will
     * be encapsulated as an `Uncomputable` value.
     *
     * For CUIDs generated using version `CUIDv2`, the availability of the timestamp depends on
     * whether it was generated internally by this class. If the timestamp cannot be computed
     * or was not saved during the generation of the instance, an exception of type
     ...
     * @since 6.1.0
     */
    val instant: Either<Uncomputable, Instant>
        get() = timestamp.map(::Instant)
    ...
    companion object {
        ...
        /**
         * Converts the current `CharSequence` into a `Cuid` instance, encapsulated within an `Either`.
         * If the conversion fails due to an invalid format or any underlying error, it will return
         * an `InvalidFormatOfType` describing the failure.
         *
         * @return An `Either` containing either the successfully created `Cuid` or an `InvalidFormatOfType`
