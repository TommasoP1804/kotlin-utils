...
internal object PantoneConverter {
    ...
    fun byName(name: String): String? {
        ...
        node.forEach { if (it.get("name").asString() == name) return it.get("hex").toString().replace("\"", "") }
        return null
    }

    fun byHex(hex: String): String? {
        val node = MAPPER.readTree(json)
        node.forEach { if (it.get("hex").asString() == hex) return it.asString().replace("\"", "") }
        return null
    }

    fun byHexGiveName(hex: String): String? {
        val node = MAPPER.readTree(json)
        node.forEach { if (it.get("hex").asString() == hex) return it.get("name").asString().replace("\"", "") }
        return null
    }
}